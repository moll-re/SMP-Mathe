...
\item \textbf{Waagerechte Asymptote}: Konvergiert $f$ f"ur $x\rightarrow \infty$ gegen eine reelle Zahl $g\in \R$, das hei�t $\indent \qquad\lim\limits_{x \rightarrow x_{0^{+}}}{f(x)}=g$.\\
...
\begin{Definition}
Eine stetige Funktion $f$ mit $a,b\in I ''eingeschlossen in'' D_{f}$ ist :\\
\begin{enumerate}
\item ... auf dem Intervall $I$ monoton wachsend, wenn $\forall a < b: f(a)? f(b)$ 
\item ... auf dem Intervall $I$ monoton fallend, wenn $\forall a < b: f(a)? f(b)$ \\
\end{enumerate}
...
\begin{enumerate}
\item monoton wachsend auf $I$ ist, dann ist $f'(x)?0$, $\forall x \in I$
\item monoton fallend auf $I$ ist, dann ist $f'(x)?0$, $\forall x \in I$
\item konstant auf $I$ ist, dann ist $f'(x)=0$, $\forall x \in I$
\end{enumerate}
