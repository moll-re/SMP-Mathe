...
\end{enumerate}
Stetigkeit ist eine lokale Eigenschaft. Die Funktion $f$ heiï¿½t dann stetig, wenn sie an jeder Stelle ihrer Definitionsmenge stetig ist.
...
beim Vereinfachen von gebrochenrationalen Funktionen ist Vorsicht geboten, denn eine hebbare Definitionslï¿½cke ''aufzuheben" ver"andert den Definitionsbereich der Funktion. Die daraus resultierende Funktion wird \textbf{stetige Fortsetzung} genannt.
...
Die Funktion heiï¿½t differenzierbar, wenn sie $\forall x \in D$ differenzierbar ist.\\
\\
Die Funktion $f(x)=|x|$ ist nicht differenzierbar, da bei der Stelle $x_{0}=0$ der linksseitige Grenzwert des differenzenquotienten ($\lim\limits_{h \rightarrow 0^{-}}{f'(x_{0})=-1$}) nicht mit dem rechtsseitigen Grenzwert (1) ï¿½bereinstimmt.\\
...
Eine Asymptote ist eine Gerade oder Kurve, die sich dem Graphen einer Funktion immer weiter ann"ahert. Dabei unterscheidet man verschiedene Fälle:\\
\begin{enumerate}
...
\item \textbf{Waagerechte Asymptote}: Konvergiert $f$ f"ur $x\rightarrow \infty$ gegen eine reelle Zahl $g\in \R$, das heißt $\lim\limits_{x \rightarrow x_{0^{+}}}{f(x)}=g$.\\
...
Bei einer gebrochenrationalen Funktion ist der ''Fehlergrad''???(wie nennt man das??), das heißt der Abstand von $g$ zu $f$ durch den Rest der Polynomdivision von Z"ahler mit Nenner gegeben.\\
