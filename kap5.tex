...
\section{Ebenen}
    ...
    \subsection{Lagebeziehungen zwischen Ebenen und Geraden}
        ...
        \begin{center}
        \begin{minipage}[t]{0.5\textwidth}
        $3)$ Zudem kann g \textbf{in E liegen}
        \begin{center}
            \begin{tikzpicture}[>=triangle 45,scale=0.6]
        ...
        Die Lagebeziehung zwischen einer Ebene $E$ und einer Geraden $g$ lässt sich wie folgt ermitteln:

        \begin{center}
            \begin{tikzpicture}[>=triangle 45,scale=0.6]
                \draw (0,0) node{$\textcolor{red}{\vv{n}} \cdot \textcolor{red}{\vv{u}} = 0?$};
                \draw [line width=0.8pt,color=gray] (-5,-0.3) -- (-4,-0.3);
                \draw [line width=0.8pt,color=gray] (-3.1,-0.3) -- (3.1,-0.3);
                \draw [line width=0.8pt,color=gray] (4,-0.3) -- (5,-0.3);
                \draw (-3.55,-0.3) node {$=$};
                \draw (3.55,-0.3) node {$\neq$};
                \draw [->,line width=0.8pt,color=gray](-5,-0.3) -- (-5,-2) node[below]{$E\parallel g$};
                \draw [->,line width=0.8pt,color=gray](5,-0.3) -- (5,-2) node[below]{$E\cap g = I$};
                ...
                \draw (-7.25,-4) node{$\in$};
                \draw (-2.75,-4) node{$\notin$};
                \draw [->,line width=0.8pt,color=gray] (-8,-4) -- (-8,-5) node[below]{$g\in E$};
                \draw [->,line width=0.8pt,color=gray] (-2,-4) -- (-2,-5) node[below]{$E\parallel g$};
            \end{tikzpicture}
        \end{center}
    ...
    \subsection{Lagebeziehungen zwischen Ebenen}
        ...
        Ebenen teilen bezüglich ihrer Lage zueinander eine Eigenschaft mit einer Ebene und einer Geraden.
        ...
        Die Lagebeziehung zwischen zwei Ebenen $E_1$ und $E_2$ lässt sich wie folgt ermitteln:
        ...
        \begin{center}
            \begin{tikzpicture}[>=triangle 45]
                ...
                \draw (-3.55,-0.3) node {$\exists t$};
                \draw (3.55,-0.3) node {$\nexists t$};
                \draw [->,line width=0.8pt,color=gray](-5,-0.3) -- (-5,-2) node[below]{$E_1\parallel E_2$};
                \draw [->,line width=0.8pt,color=gray](5,-0.3) -- (5,-2) node[below]{$E_1\cap E_2 = g;g:\vv{x}=\vv{q} + r \cdot \vv{u};r\in\R$};
                \draw (-5,-2.8) node{$\textcolor{blue}{Q_{E_2}}\in E_1 \lor \textcolor{blue}{Q_{E_1}}\in E_2?$};
                \draw [line width=0.8pt,color=gray] (-5,-3) -- (-5,-4);
                \draw [line width=0.8pt,color=gray] (-8,-4) -- (-7.6,-4);
                \draw [line width=0.8pt,color=gray] (-6.9,-4) -- (-3.1,-4);
                \draw [line width=0.8pt,color=gray] (-2.4,-4) -- (-2,-4);
                \draw (-7.25,-4) node{$\in$};
                \draw (-2.75,-4) node{$\notin$};
                \draw [->,line width=0.8pt,color=gray] (-8,-4) -- (-8,-5) node[below]{$E_1\equiv E_2$};
                \draw [->,line width=0.8pt,color=gray] (-2,-4) -- (-2,-5) node[below]{$E_1\parallel E_2$};
            \end{tikzpicture}
        \end{center}

        Für die Berechnung der Schnittgeraden gibt es unterschiedliche Ansätze abhängig von der
    ...
    \subsection{Winkel zwischen Ebenen (und Geraden)}
        ...
        Die einzige bisher angesprochene Möglichkeit den Winkel zwischen zwei geometrischen Formen zu
        ermitteln macht sich der Definition des Skalarprodukts zunutze. Wie bereits erläutert gilt:
        $\cos(\alpha)=\dfrac{\vv{v}\cdot\vv{u}}{|\vv{v}|\cdot|\vv{u}|}$. Hierbei soll erneut hervorgehoben werden, dass
        ...
        \begin{enumerate}
            \item Geraden: $\cos(\alpha) = \dfrac{\textcolor{red}{\vv{u_1}} \cdot \textcolor{red}{\vv{u_2}}} {|\textcolor{red}{\vv{u_1}}| \cdot |\textcolor{red}{\vv{u_2}}|}$
            \item Ebenen: $\cos(\alpha) = \dfrac{\textcolor{orange}{\vv{n_1}} \cdot \textcolor{orange}{\vv{n_2}}} {|\textcolor{orange}{\vv{n_1}}| \cdot |\textcolor{orange}{\vv{n_2}}|}$
            \item Gerade / Ebene: $\sin(\alpha) = \dfrac{\textcolor{red}{\vv{u}} \cdot \textcolor{orange}{\vv{n}}} {|\textcolor{red}{\vv{u}}| \cdot |\textcolor{orange}{\vv{n}}|}$
        \end{enumerate}
        \begin{center}
            \definecolor{qqwuqq}{rgb}{0,0.4,0}
            \begin{tikzpicture}[>=triangle 45]
    ...
    \subsection{Abstand zu einem Punkt}
        ...
        Eine simple und intuitive Art den Abstand einer Ebene $E$ zu einem Punkt $P$ zu bestimmen, wäre eine Gerade
        ...
        \begin{Definition}[Hess'sche Normalenform]
            ...
            $E_h: \dfrac{\textcolor{red}{\vv{n}} \cdot [\vv{x} - \textcolor{blue}{\vv{q}}]}{|\textcolor{red}{\vv{n}}|} = 0$
        \end{Definition}
        ...
        \begin{center}
            ...
            \begin{tikzpicture}[>=triangle 45]
                \draw[->,line width=0.8pt,color=red] (-2,0,2) -- (-2,-2.5,2);
                \draw[line width=0.1pt,color=red,fill=red,fill opacity=0.2] (-4,0,-4) -- (-4,0,4) -- (4,0,4) -- (4,0,-4) -- cycle;

                \draw[line width=0.1pt,color=blu,fill=blu,fill opacity=0.2] (-2,0,2) -- (-1,0.65,2) arc (20:85:1.15) -- cycle;
                \draw[line width=0.1pt,color=blu,fill=blu,fill opacity=0.2] (2,2,-2) -- (1,1.35,-2) arc (205:268:1.15) -- cycle;

                \draw[line width=0.8pt,color=gree] (2,0,-2) -- (2,2,-2);
                \draw[->,line width=0.8pt,color=black] (-2,0,2) -- (2,2,-2);
                \draw[->,line width=0.8pt,color=red] (-2,0,2) -- (-2,2.5,2);
                \draw[line width=0.8pt,color=black,dashed] (-2,0,2) -- (2,0,-2);

                \draw[line width=0.8pt,color=gree] (2,0,-2) -- (2,-2,-2);
                \draw[->,line width=0.8pt,color=black] (-2,0,2) -- (2,-2,-2);

                \draw[line width=1pt] (-2,0.5,2) arc (90:180:0.5);
                \draw[line width=1pt] (2,0.5,-2) arc (90:180:0.5);
                \draw[line width=1pt] (-2.2,0.2,2) circle (0.5pt);
                \draw[line width=1pt] (1.8,0.2,-2) circle (0.5pt);

                \draw[line width=0.8pt,color=black,fill=black,fill opacity=0.4] (2,0,-2) circle (2pt);
                \draw[line width=0.8pt,color=black,fill=black,fill opacity=0.4] (2,2,-2) circle (2pt);
                \draw[line width=0.8pt,color=black,fill=black,fill opacity=0.4] (-2,0,2) circle (2pt);

                \draw[line width=0.8pt,color=black,fill=black,fill opacity=0.4] (2,-2,-2) circle (2pt);

                \draw[line width=0.8pt,color=blue] (-2.1,0.1,2.7) node {$Q$};
                \draw[line width=0.8pt,color=red] (-2.25,1.25,2) node {$\vv{n}$};
                \draw[line width=0.8pt,color=red] (-2.35,-1.25,2) node {$-\vv{n}$};
                \draw[line width=0.8pt,color=gree] (2.2,1,-2) node {$d$};
                \draw[line width=0.8pt,color=gree] (2.2,-1,-2) node {$d$};
                \draw[line width=0.8pt,color=red] (2.6,0.2,-1.2) node {$F$};
                \draw[line width=0.8pt,color=red] (2.3,2.15,-2) node {$P$};
                \draw[line width=0.8pt,color=red] (2.3,-2.15,-2) node {$P$};
                \draw[line width=0.8pt,color=red] (3.7,0,-4.5) node {$E$};
                \draw[line width=0.8pt,color=blu] (-1.6,0.8,2) node {$\alpha$};
                \draw[line width=0.8pt,color=blu] (1.6,1.25,-2) node {$\alpha$};
            \end{tikzpicture}
        \end{center}

        \definecolor{gree}{rgb}{0,0.4,0}
        Anhand dieser Graphik lässt sich die Formel zur Berechnung des Abstands eines Punktes zu einer Ebene ablesen:
        $\textcolor{gree}{d} = \dfrac{|\textcolor{red}{\vv{n}} \cdot (\vv{p} - \textcolor{blue}{\vv{q}})|}{|\textcolor{red}{\vv{n}}|}$

        \begin{Beweis}
            \definecolor{gree}{rgb}{0,0.4,0}
            \underline{$\vv{FP} = r \cdot \textcolor{red}{\vv{n}} ; r \in \R^{+}$}:
            
            Nach Theoremen der Trigonometrie und der Definition des Skalarprodukts gilt:
            \begin{align*}
                \qquad \qquad cos(\alpha) &= \dfrac{\textcolor{green}{d}}{|\vv{\textcolor{blue}{Q}P}|} \\
                              cos(\alpha) &= \dfrac{\textcolor{red}{\vv{n}} \cdot \vv{\textcolor{blue}{Q}P}}{|\textcolor{red}{\vv{n}}| \cdot |\vv{\textcolor{blue}{Q}P}|} \\
            \end{align*}
            Daraus folgt:
            \begin{align*}
                \qquad \qquad \dfrac{\textcolor{green}{d}}{|\vv{\textcolor{blue}{Q}P}|} &= \dfrac{\textcolor{red}{\vv{n}} \cdot \vv{\textcolor{blue}{Q}P}}{|\textcolor{red}{\vv{n}}| \cdot |\vv{\textcolor{blue}{Q}P}|} \\
                                                                                     d &= \dfrac{\textcolor{red}{\vv{n}} \cdot \vv{\textcolor{blue}{Q}P}}{|\textcolor{red}{\vv{n}}|} \\
                                                                                     d &= \dfrac{\textcolor{red}{\vv{n}} \cdot (\vv{p} - \vv{\textcolor{blue}{q}})}{|\textcolor{red}{\vv{n}}|} \\
            \end{align*}
            ...
            \end{align*}
            \\
            Somit lautet die gesuchte Formel: $\textcolor{gree}{d} = \dfrac{|\textcolor{red}{\vv{n}} \cdot (\vv{p} - \textcolor{blue}{\vv{q}})|}{|\textcolor{red}{\vv{n}}|}$
        \end{Beweis}
...
\section{Kreise und Sphären}
    ...
    Letzte relevant Objektgruppe ist die der Kreise und Sphären, wobei Letztere lediglich auf den $\R^3$ angewandte Formen der Ersten sind.

    \subsection{Kreise}

        Kreise werden über einen \textbf{Mittelpunkt} und einen \textbf{Radius} definiert. Alle Punkte, welche einen Distanz gleich dem Radius zum Mittelpunkt aufweisen, sind teil der Kreismenge, die den Kreis algebraisch beschreibt. Wie bei allen anderen Sektoren der Geometrie ist es von Nöten, das Ganze zu visualisieren:
        ...
        \begin{center}
            ...
            \begin{tikzpicture}[>=triangle 45]
                ...
                \draw[line width=0.8pt,color=purp] (2.33,2.63) node {$r$};
                ...
                \draw[line width=0.8pt,color=purp] (-2.8,-3.3) node {$M_2 (-3,-3)$};
                \draw[->,line width=0.8pt,color=purp] (-3,-3) -- (-4.05,-1.30);
                \draw[line width=0.8pt,color=purp] (-3.33,-2.05) node {$r$};
            \end{tikzpicture}
        \end{center}

        Hieraus ergibt sich eine einfache Formel, um einen Kreis zu definieren, aus welcher auch die allgemeine Kreisgleichung folgt:
        ...
        \end{align*}
        Wie die Graphik es bereits verdeutlicht, kann es von Nutzen sein mit einer Kreisscheibe zu arbeiten. Diese wird über die Menge der Punkte, die innerhalb des Kreises liegen, definiert.
...
\subsubsection{Tangenten an einen Kreis}
            ...
            \begin{Definition}
                Eine Tangente an einen Punkt eines Kreises ist vergleichbar mit der an einen Punkt einer Kurve. Sie ist gerade zu der Geraden equivalent, die \textbf{den Kreis in diesem Punkt berührt}. Dies impliziert auch, dass es nur eine Menge identischer Geraden gibt, welche erfolgreich als Tangente kandidieren können.
            \end{Definition}
            ...
            \begin{center}
                ...
                \begin{tikzpicture}[>=triangle 45]
                    ...
                    \draw[line width=1.5pt,color=gray] (0,6.12) -- (6.12,0);
                    \draw[->,line width=1.5pt,color=red] (3.06, 3.06) -- (4.56,1.56);
                    \draw[line width=0.8pt] (2.7,2.7) arc (-140:-45:0.5);
                    \draw[line width=0.8pt] (3.1,2.8) circle (0.5pt);
                    \draw[line width=0.8pt,color=gray] (5.3,1.2) node {$t_1$};
                    \draw[line width=0.8pt,color=gray] (-2.8,-0.8) node {$X$};
                    ...
                    \draw[line width=1.5pt,color=gray] (-7.05,-3.4) -- (-1.75,0.3);
                    \draw[->,line width=1.5pt,color=red] (-4.05,-1.3) -- (-2.9,-0.5);
                    \draw[line width=0.8pt] (-3.8,-1.7) arc (-60:30:0.5);
                    \draw[line width=0.8pt] (-3.75,-1.35) circle (0.5pt);
                    \draw[line width=0.8pt,color=gray] (-6.5,-2.7) node {$t_2$};
                    \draw[line width=0.8pt,color=gray] (4.4,1.3) node {$X$};
                \end{tikzpicture}
            \end{center}

            Erneut liefert uns die Graphik eine generelle Formel:
            
            \begin{align*}
                \qquad \qquad \qquad &\vv{MP} \cdot \vv{PX} = 0 ; P \in M_K, X \in M_T \text{ mit $M_K$ und $M_T$ Kreis- und Tangentenmenge} \\
                     \Leftrightarrow &(\vv{p} - \vv{m}) \cdot (\vv{x} - \vv{p}) = 0 \quad | + (\vv{p} - \vv{m})^2 \\
                     \Leftrightarrow &(\vv{p} - \vv{m}) \cdot (\vv{x} - \vv{p}) + (\vv{p} - \vv{m})^2 = (\vv{p} - \vv{m})^2 \\
                     \Leftrightarrow &(\vv{p} - \vv{m}) \cdot ((\vv{x} \cancel{- \vv{p}}) + (\cancel{\vv{p}} - \vv{m})) = (\vv{p} - \vv{m})^2 \\
                     \Leftrightarrow &(\vv{p} - \vv{m}) \cdot (\vv{x} - \vv{m}) = r^2 \\
            \end{align*}

        \subsubsection{Polare an einen Kreis}

            \begin{Definition}
                Sei $\mathscr{C}$ ein Kreis mit Mittelpunkt $M$ und $P$ ein Punkt mit $|\vv{MP}| \nleq r$. Des Weiteren seien die zwei Tangenten(mengen) an den Kreis, die durch den Punkt $P$ gehen, gegeben. Dann bezeichnet man die Gerade, welche die Berührpunkte der Tangenten beinhaltet, als \textbf{Polare an den Kreis $\mathscr{C}$ zum Punkt P}.
            \end{Definition}
            ...
            Die Prozedur sollte allmählich bekannt vorkommen:

            \begin{center}
                \definecolor{purp}{rgb}{0.5,0,0.5}
                \definecolor{lightblue}{rgb}{0,0.7,0.7}
                \begin{tikzpicture}[>=triangle 45]
                    \draw[line width=0.8pt,color=lightblue,fill=lightblue,fill opacity=0.3] (0,0) arc (0:360:3);
                    \draw[line width=0.8pt] (5,0) arc (0:360:4);
                    \draw[line width=0.8pt] (-7,0) -- (7,0);
                    \draw[->,line width=0.8pt,color=purp] (-3,0) -- (-1.85,2.75);
                    \draw[line width=0.8pt] (-2.05,2.25) arc (-110:-20:0.5);
                    \draw[line width=0.8pt] (-1.78,2.52) circle (0.5pt);
                    \draw[->,line width=0.8pt,color=purp] (-3,0) -- (-1.85,-2.75);
                    \draw[line width=0.8pt] (-2.05,-2.25) arc (110:20:0.5);
                    \draw[line width=0.8pt] (-1.78,-2.52) circle (0.5pt);
                    \draw[line width=0.8pt] (1,0) -- (-1.85,2.75);
                    \draw[line width=0.8pt] (1,0) -- (-1.85,-2.75);
                    \draw[line width=0.8pt,color=purp] (-3.3,-0.25) node {$M$};
                    \draw[line width=0.8pt,color=purp,fill=purp] (-3,0) circle (2pt);
                    \draw[line width=0.8pt] (1.3,-0.25) node {$M_T$};
                    \draw[line width=0.8pt,fill=black] (1,0) circle (2pt);
                    \draw[line width=0.8pt,color=purp] (5.2,0.4) node {$P$};
                    \draw[line width=0.8pt,fill=black] (-1.85,2.75) circle (2pt);
                    \draw[line width=0.8pt,color=purp] (-1.5,2.9) node {$B$};
                    \draw[line width=0.8pt,color=purp,fill=purp] (5,0) circle (2pt);
                    \draw[line width=0.8pt,color=purp] (-2.225,1.175) node {$r$};
                    \draw[line width=0.8pt] (-0.625,1.175) node {$r_T$};
                    \draw[line width=0.8pt,color=blue] (-1.65,-4.5) node {$p$};
                    \draw[line width=0.8pt,color=blue] (-1.57,4) node {$X$};
                    \draw[line width=0.8pt,fill=black] (-1.85,3.9) circle (2pt);
                    \draw[line width=1pt] (6.167,-0.475) -- (-4.333,3.8);
                    \draw[line width=1pt] (6.167,0.475) -- (-4.333,-3.8);
                    \draw[line width=1pt,color=blue] (-1.85,-5) -- (-1.85,5);
                    \draw[line width=0.8pt] (-1.35,0) arc (0:90:0.5);
                    \draw[line width=0.8pt] (-1.63,0.2) circle (0.5pt);
                \end{tikzpicture}
            \end{center}

            Wenn auch ein wenig um mehr Ecken gedacht als üblich:
            
            \begin{align*}
                \qquad \qquad \qquad &\left|\begin{array}{c} \vv{MB} \cdot \vv{PB} = 0 \\ \vv{MP} \cdot \vv{BX} = 0 \end{array}\right| \\
                     \Leftrightarrow &\left|\begin{array}{c} (\vv{b} - \vv{m}) \cdot (\vv{b} - \vv{p}) = 0 \\ (\vv{p} - \vv{m}) \cdot (\vv{x} - \vv{b}) = 0 \end{array}\right| \qquad \text{Nach der bereits aufgestellten Formel für Tangenten} \\
                     \Leftrightarrow &\left|\begin{array}{c} (\vv{b} - \vv{m}) \cdot (\vv{p} - \vv{m}) = r^2 \\ (\vv{p} - \vv{m}) \cdot (\vv{x} - \vv{b}) = 0 \end{array}\right| \qquad (1) + (2) \\
    ...
    \subsection{Sphären}
        ...
        Sphären werden wie Kreise über einen \textbf{Mittelpunkt} und einen \textbf{Radius} definiert. Alle Punkte, welche einen Distanz gleich dem Radius zum Mittelpunkt aufweisen, sind teil der Sphärenmenge, die die Sphäre algebraisch beschreibt:

        \begin{center}
            \definecolor{purp}{rgb}{0.5,0,0.5}
            \definecolor{lightblue}{rgb}{0,0.7,0.7}
            \begin{tikzpicture}[>=triangle 45]
                \draw[line width=0.8pt] (-5,0,0) arc (180:360:5cm and 2.5cm);
                \draw[line width=0.8pt,dashed] (-5,0,0) arc (180:0:5cm and 2.5cm);
                \draw[line width=0.8pt] (0,5,0) arc (90:270:2.5cm and 5cm);
                \draw[line width=0.8pt,dashed] (0,5,0) arc (90:-90:2.5cm and 5cm);
                \draw[line width=0.8pt] (0,0,0) circle (5cm);
                \shade[ball color=lightblue,opacity=0.3] (0,0,0) circle (5cm);
                \draw[line width=0.8pt] (4,4,0) node {$\mathscr{C}$};
                \draw[line width=0.8pt,color=purp] (-0.3,0.3,0) node {$M$};
                \draw[line width=0.8pt,color=purp,fill=purp] (0,0,0) circle (2pt);
                \draw[line width=0.8pt,color=purp] (1.3,1.6,1.4435) node {$r$};
                \draw[->,line width=0.8pt,color=purp] (0,0,0) -- (2.887,2.887,2.887);
                \draw[line width=0.8pt,color=purp] (3.2,3.2,3.2) node {$P$};
                \draw[line width=0.8pt,color=blue] (0,5,0) arc (90:-90:1.9cm and 5cm);
                \draw[line width=0.8pt,color=blue,dashed] (0,5,0) arc (90:270:1.9cm and 5cm);
            \end{tikzpicture}
        \end{center}

        \begin{Theorem}[Sphärengleichung]
            Eine Sphäre im $\R^n ; n \geq 3$ mit Mittelpunkt $M$ und Radius $r$ ist eindeutig über die folgende Gleichung definiert:
                                    $$|\vv{MP}| = r ; P \in M_{\mathscr{C}}$$
            Die allgebraische Formel lautet somit wie folgt:
                                    $$(x-a)^2 + (y-b)^2 + (z-c)^2 = r^2 ; (x,y,z) = (p_x,p_y,p_z) \land (a,b,c) = (m_x,m_y,m_z)$$
        \end{Theorem}
        ...
        \subsubsection{Tangenten an eine Sphäre}
            ...
            \begin{Definition}
                Die Tangente in einem Punkt an eine Sphäre unterscheidet sich nur in einer Eigenschaft von der an einen Kreis: mehrere nicht identische Geraden berühren eine Sphäre in einem Punkt. Tatsächlich stellt diese Menge von Geraden eine Ebene dar: die \textbf{Tangentialebene}.
            \end{Definition}
            ...
            \begin{center}
                ...
                \begin{tikzpicture}[>=triangle 45]
                    ...
                    \draw[line width=0.8pt] (0,0,0) circle (5cm);
                    \shade[ball color=lightblue,opacity=0.3] (0,0,0) circle (5cm);
                    \draw[line width=0.8pt] (3,5,0) node {$\mathscr{C}$};
                    \draw[line width=0.8pt,color=purp] (-0.3,0.3,0) node {$M$};
                    \draw[line width=0.8pt,color=purp,fill=purp] (0,0,0) circle (2pt);
                    \draw[line width=0.8pt,color=purp] (1.3,1.6,1.4435) node {$r$};
                    \draw[->,line width=0.8pt,color=purp] (0,0,0) -- (2.887,2.887,2.887);
                    ...
                    \draw[line width=0.8pt,color=blue] (0,5,0) arc (90:-90:1.9cm and 5cm);
                    \draw[line width=0.8pt,color=blue,dashed] (0,5,0) arc (90:270:1.9cm and 5cm);
                    \draw[line width=0.4pt,color=orange] (11.548,-5.774,11.548) -- (-5.774,11.548,-5.774);
                    \draw[line width=0.4pt,color=orange] (-5.774,11.548,11.548) -- (11.548,-5.774,-5.774);
                    \draw[line width=0.4pt,color=orange] (-5.774,-5.774,-5.774) -- (11.548,11.548,11.548);
                    \draw[line width=0.1pt,color=orange,fill=orange,fill opacity=0.1] (8.661,-2.887,8.661) -- (-2.887,8.661,8.661) --     (-2.887,-2.887,-2.887) -- (-2.887,8.661,-2.887) -- (8.661,-2.887,-2.887) -- (8.661,8.661,8.661) -- cycle;
                    \draw[line width=0.8pt] (-3,-8,0) node {\begin{tabular}{c} Wenn ein harmloses Versehen eines Menschen in \glqq wahrer Schönheit\grqq \text{ } endet... \\ \linebreak Spaß beiseite, ich habs einfach mal so gelassen \\ \end{tabular}};
                \end{tikzpicture}
            \end{center}

            \begin{Theorem}[Tangentialebene]
                Für die Tangentialebene an eine Sphäre $\mathscr{C}$, mit Mittelpunkt $M$ und Radius $r$, in einem Punkt $P$ gilt:
                                                    $$|\vv{MP}| = r$$
            \end{Theorem}
        ...
        \subsubsection{Polarebene?}
            ...
            \begin{center}
                ...
                \begin{tikzpicture}[>=triangle 45]
                    ...
                    \draw[line width=0.8pt,dashed] (-5,0,0) arc (180:0:5cm and 2.5cm);
                    \draw[line width=0.8pt,dashed] (0,5,0) arc (90:-90:2.5cm and 5cm);
                    \draw[line width=0.8pt] (7.071,0,-7.071) -- (3.5355,0,-3.5355);
                    \draw[line width=0.8pt,dashed] (3.5355,0,-3.5355) -- (0,0,0);
                    \draw[line width=0.8pt] (7.071,0,-7.071) -- (2.7105,1.175,-3.5355);
                    \draw[line width=0.8pt,dashed] (2.7105,1.175,-3.5355) -- (-1.65,2.35,0);
                    \shade[top color=blue!40!white,opacity=0.75] (0,5,0) arc (90:-90:1.9cm and 5cm) -- (7.071,0,-7.071) -- cycle;
                    \shade[top color=blue!40!white,opacity=0.75] (0,5,0) arc (90:-270:1.9cm and 5cm);
                    \shade[top color=blue!40!white,opacity=0.75] (0,5,0) arc (90:-270:1.9cm and 5cm);
                    \draw[line width=0.8pt] (-5,0,0) arc (180:360:5cm and 2.5cm);
                    \draw[line width=0.8pt] (0,5,0) arc (90:270:2.5cm and 5cm);
                    \draw[line width=0.8pt] (0,0,0) circle (5cm);
                    \draw[line width=0.8pt] (4,4,0) node {$\mathscr{C}$};
                    \draw[line width=0.8pt,color=purple] (-0.3,0.3,0) node {$M$};
                    \draw[line width=0.8pt,color=purple,fill=purple] (0,0,0) circle (2pt);
                    \draw[line width=0.8pt,color=purple] (1.3,1.6,1.4435) node {$r$};
                    \draw[->,line width=0.8pt,color=purple] (0,0,0) -- (2.887,2.887,2.887);
                    %\draw[line width=0.8pt,color=purple] (3.2,3.2,3.2) node {$P$};
                    \draw[line width=0.8pt,color=blue] (0,5,0) arc (90:-90:1.9cm and 5cm);
                    \draw[line width=0.8pt,color=blue,dashed] (0,5,0) arc (90:270:1.9cm and 5cm);
                    \draw[line width=0.8pt] (7.271,0.2,-7.071) node {$P$};
                    \draw[line width=0.8pt] (7.071,0,-7.071) -- (1.65,-2.35,0);
                    \draw[line width=0.8pt] (7.071,0,-7.071) -- (1.55562,3.9,-1.55562);
                    \draw[line width=0.8pt,dashed] (1.55562,3.9,-1.55562) -- (0,5,0);
                    \draw[line width=0.8pt] (7.071,0,-7.071) -- (3.5355,-2.5,-3.5355);
                    \draw[line width=0.8pt,dashed] (3.5355,-2.5,-3.5355) -- (0,-5,0);
