...
\begin{Beweis}
	...
	\begin{enumerate}
		\item Monotonieverhalten:
			...
			\begin{tabular}{rlcl}
				$\A x<1$: & $f'(x)>0$ & & $\Leftrightarrow f \nearrow$\\
				für $x=1$: & $f'(x)=0$ & und VZW $+\rightarrow-$ & $\Leftrightarrow$ Hochpunkt\\
				$\A x>1$: & $f'(x)<0$ & & $\Leftrightarrow f \searrow$\\
			\end{tabular}
		...
		\item Graph von f:
			...
			\draw [color=cqcqcq,dash pattern=on 1pt off 1pt, xstep=0.5cm,ystep=0.5cm] (-0.5,-2) grid (6,2);
			\draw[->,color=black] (-0.5,0) -- (6,0);
			\foreach \x in {-0.5,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5,5.5}
			\draw[shift={(\x,0)},color=black] (0pt,2pt) -- (0pt,-2pt) node[below] {\footnotesize $\x$};
			\draw[->,color=black] (0,-2) -- (0,2);
			\foreach \y in {-2,-1.5,-1,-0.5,0.5,1,1.5}
			\draw[shift={(0,\y)},color=black] (2pt,0pt) -- (-2pt,0pt) node[left] {\footnotesize $\y$};
			\draw[color=black] (0pt,-10pt) node[right] {\footnotesize $0$};
			\clip(-0.5,-2) rectangle (6,2);
			\draw[line width=1.6pt,color=qqqqcc, smooth,samples=100,domain=-0.5:6.0] plot(\x,{(\x)*2.718281828^(1-(\x))});
			\end{tikzpicture}
