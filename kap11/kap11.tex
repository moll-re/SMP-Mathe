...
\begin{center}
  \begin{tabular}{r c l l}
    ...
    \( \displaystyle\int_a^b r*f(x)dx\)  & $=$ &  r*\( \displaystyle\int_a^b f(x)dx\) & Linearität\\
...
\begin{Definition}
  ...
  \( \displaystyle\int_a^b \pi f^2(z)dz\)
  hat.\\
...
\end{Beispiel}
\begin{Bemerkung}
Für Funktionen wie $f(x)= \dfrac{1}{\sqrt x}$ existiert kein Uneigentliches Integral.\\Stimmt nicht, sieht aber als Platzhalter gut aus.
\end{Bemerkung}
...
Diese sollten ohne weitere Rechtfertigung oder Beweis verwendet werden dürfen.
\begin{align*}
