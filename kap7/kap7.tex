...
\subsection{Sigmaumgebungen}
\subsubsection{Einschub: Normalverteilung}
\begin{Definition}
Eine stetige Zufallsvariable $X$ hat eine (Gauß- oder) Normalverteilung mit Erwartungswert $\mu$ und Varianz $\sigma ^{2}$, oft geschrieben als $X\sim \N\left(\mu ,\sigma ^{2}\right)$, wenn $X$ die folgende Wahrscheinlichkeitsdichte hat:
$$\varphi(x)=f(x\mid \mu ,\sigma ^2) = \dfrac {1}{\sqrt {2\pi \sigma^2}}} \cdot e^{- \dfrac{1}{2} {\left(\dfrac{x-\mu}{\sigma}\right)}^2} \qquad ; -\infty <x<\infty$$
...
\begin{Definition}
  ...
  $$\Phi(x) = \dfrac{1}{\sqrt{2\pi}}\int_{-\infty}^{x} e^{-\frac{1}{2}t^2} dt$$
\end{Definition}
\begin{Bemerkung}
  Diese Definition ist somit analog zur Definition der Verteilungsfunktion der Binomialverteilung (Summe der Flächen).\\
...
\begin{Beweis}
  \begin{align*}
    f(\mu - \sigma \mid \mu ,\sigma ^2) & = & \dfrac{1}{\sqrt{2\pi \sigma^2}} \cdot e^{- \dfrac{1}{2} {\left(\dfrac{x-\mu}{\sigma}\right)}^2}\\
    &=& \dfrac {1}{\sqrt {2\pi \sigma^2}}}e^{\frac{1}{2}}\\
    &=& \dfrac {1}{\sqrt {2\pi (\sqrt{np(1-p)})^2}}}e^{\frac{1}{2}}\\
    &=& \dfrac {1}{\sqrt {2\pi np(1-p)}}e^{\frac{1}{2}}\\
  \end{align*}?????
\end{Beweis}
...
\subsubsection{Anwendung: Binomialverteilung}
\begin{Theorem}[- Laplace-Bedingung]
  ...
  $$B_{l,p}(k) = P(X=k) = {l \choose k} p^k (1-p) ^{l-k} \equiv \varphi(k) = \dfrac {1}{\sqrt {2\pi \sigma^2}}} \cdot e^{- \dfrac{1}{2} {\left(\dfrac{x-\mu}{\sigma}\right)}^2}$$
\end{Theorem}
\begin{Beweis}
